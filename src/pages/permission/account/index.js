...
import {Button, Input, Table, Modal, message, Select, Form, Checkbox} from 'antd/lib/index';
import { queryAll, queryAllRole, registerUser, updateUser, deleteUser } from "./action";
import moment from 'moment'
import './index.less'
import {couponDelete} from "../role/action";
const { Option } = Select;
export default class Index extends React.Component {
    ...
    render() {
        ...
        const columns = [
            { title: '账号', dataIndex: 'username', width: 120, align: 'center' },
            { title: '名称', dataIndex: 'name', width: 120, align: 'center' },
            { title: '角色', dataIndex: 'roleName', width: 120, align: 'center' },
            { title: '所属部门', dataIndex: 'dept', width: 120, align: 'center' },
            { title: '最后登录时间', dataIndex: 'lastLoginTime', width: 120, align: 'center',
                render: (value, record) => <span>{record.lastLoginTime ? moment(record.lastLoginTime).format('YYYY-MM-DD') : ''}</span>
            },
            {
            ...
            { title: '状态', dataIndex: 'isActive', width: 120, align: 'center',
