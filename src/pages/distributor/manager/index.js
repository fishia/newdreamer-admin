...
import {
  requestForFabricList,
  requestForFabricEdit,
  requestForFabricCreate,
  ...
  const [columns] = useState([
    {
      ...
      dataIndex: 'wechatId',
      onRead: true,
      render: text => <span style={{ color: '#468EFE' }}>{text}</span>,
    },
    ...
    { title: '订单总金额', dataIndex: 'orderAmount' },
    {
      ...
      render: (item, record) => (
        <div className="product-table-operations">
          <Button type="primary" size="small" onClick={() => _edit(record)}>
            修改
          </Button>
              ...
              <div className="pm-edit-item">
                ...
                {col.type === 'image' && (
                  <div className="pm-edit__images">
                    {editInfo[col.dataIndex] && (
                      <img className="pm-edit__image" alt="edit" src={editInfo[col.dataIndex]} />
                    )}
                    <Upload
                      action="/newdreamer/file/upload?FileDirectorEnum=PRODUCT"
                      method="post"
